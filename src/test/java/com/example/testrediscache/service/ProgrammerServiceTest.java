...
@ContextConfiguration(initializers = ProgrammerServiceTest.Initializer.class, classes = TestRedisCacheApplication.class)
public class ProgrammerServiceTest {
    ...
    public void shouldCacheProgrammersByMainLanguage() {
        //Given
        List<Programmer> programmers = Arrays.asList(Programmer.builder().nick(PROGRAMMER_NICK).mainLanguage(PROGRAMMER_MAIN_LANGUAGE).build());
        programmers.forEach(programmerRepository::save);

        // When
        List<Programmer> found = programmerService.findProgrammersByMainLanguage(PROGRAMMER_MAIN_LANGUAGE);

        //Then
        assertThat(found).isNotNull();
        assertThat(found).hasSize(programmers.size());
        assertThat(found.get(0)).extracting("nick", "mainLanguage").contains(PROGRAMMER_NICK, PROGRAMMER_MAIN_LANGUAGE);
    ...
    public void shouldEvictProgrammersCache() {
        //Given
        List<Programmer> programmers = Arrays.asList(Programmer.builder().nick(PROGRAMMER_NICK).mainLanguage(NEW_PROGRAMMER_MAIN_LANGUAGE).build());
        Cache programmersCache = cacheManager.getCache(PROGRAMMERS_CACHE_NAME);
        ...
        Programmer toCreate = Programmer.builder().nick(NEW_PROGRAMMER_NICK).mainLanguage(NEW_PROGRAMMER_MAIN_LANGUAGE).build();
        ...
        assertThat(saved).extracting("nick", "mainLanguage").contains(NEW_PROGRAMMER_NICK, NEW_PROGRAMMER_MAIN_LANGUAGE);
